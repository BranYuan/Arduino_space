...
void loop() {
  ...
  int left_servo_pos = map(analogRead(left_servo_pos_pin),67,943,500,2500);
  ...
  int goal_pos = handle_x;
  while(abs(left_servo_pos-handle_x)>15)
  {
    goal_pos = handle_x + 0.5*(handle_x - left_servo_pos);
    left_servo.writeMicroseconds(goal_pos);
    left_servo_pos = map(analogRead(left_servo_pos_pin),67,943,500,2500);
    handle_x = map(analogRead(handle_x_pin),0,1024,500,2500);   
    }
  ...
  int datas[]={handle_x,left_servo_pos};
